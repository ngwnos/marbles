...
import { createBumper } from './pieces/bumper-preview';
import './ramp-review.css';
import { ComponentLinks } from './component-links';

const references = [
 ['Original zigzag drawing • Fig. 3', 'https://patentimages.storage.googleapis.com/63/b5/b5/fc01ae811c51ae/US4713038-drawings-page-2.png'],
];
function App() {
  const dropTest=usePreviewDropTest('bumper');
  ...
  useEffect(() => {
    ...
    let ramp: Awaited<ReturnType<typeof createBumper>> | undefined;
    ...
    void Promise.all([renderer.init(), createBumper()]).then(([, createdRamp]) => {
      ramp = createdRamp;
  ...
  return <main>
    <section className="model">
      <header><span>PIECE 08</span><h1>Bumper ramp</h1><p>Discovery Toys Marbleworks · No. 149 · approval draft</p></header>
    ...
    <aside><ComponentLinks /><h2>Reference</h2><select value={reference} onChange={e => setReference(Number(e.target.value))}>{references.map(([title], i) => <option key={title} value={i}>{title}</option>)}</select>
      ...
      <h2>Original construction</h2><p>Eight alternating angular bends with rounded molding edges and the shared U-shaped track section. The opposite C connector is the drop.</p>
      <p><a href="https://www.ebay.com/itm/198497506138" target="_blank">No. 149: top and underside photos</a></p>
      <h2>Working dimensions</h2><p>140.1 mm connector spacing, 27 mm bodies, 13 mm fall. Bend proportions are fitted to the photographs and patent; physical measurements remain unverified.</p>
    </aside>
  </main>;
}
