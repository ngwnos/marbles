...
import { createPaddleRamp } from './pieces/paddle-preview';
import './ramp-review.css';
import { ComponentLinks } from './component-links';

const references = [
 ['No.147 • original design drawings', 'https://patentimages.storage.googleapis.com/1e/58/98/847abaaba17ee6/USD290145-drawings-page-2.png'],
 ['No.147 • curved chute and supports', 'https://i.ebayimg.com/images/g/C~IAAeSw5eRpKeyF/s-l1600.webp'],
 ['No.147 • wheel and chute', 'https://i.ebayimg.com/images/g/TvsAAeSwhQNqVmhP/s-l1600.webp'],
 ['No.147 • side elevation', 'https://i.ebayimg.com/images/g/EbQAAeSwe1RpKeyF/s-l1600.webp'],
 ['No.147 • underside', 'https://i.ebayimg.com/images/g/8cQAAeSwr3RqVmhP/s-l1600.webp'],
];
function App() {
  const dropTest=usePreviewDropTest('paddle');
  ...
  useEffect(() => {
    ...
    controls.target.set(0, 53, -5);
    controls.enableDamping = true;
    let ramp: Awaited<ReturnType<typeof createPaddleRamp>> | undefined;
    ...
    setView.current = view => {
      ...
      controls.update();
      const target = controls.target.set(0, 53, -5);
    ...
    void Promise.all([renderer.init(), createPaddleRamp()]).then(([, createdRamp]) => {
      ramp = createdRamp;
  ...
  return <main>
    <section className="model">
      <header><span>PIECE 06</span><h1>Paddle-wheel ramp</h1><p>Discovery Toys Marbleworks · No. 147 · approval draft</p></header>
    ...
    <aside><ComponentLinks /><h2>Reference</h2><select value={reference} onChange={e => setReference(Number(e.target.value))}>{references.map(([title], i) => <option key={title} value={i}>{title}</option>)}</select>
      ...
      <h2>Original construction</h2><p>A raised receiving cup feeds a curved chute beneath a twelve-pocket wheel. Two ribbed cheeks support its axle. The channel continues into the shared coiled outlet.</p>
      <p><a href="https://www.ebay.com/itm/117301918807" target="_blank">No.147 reference</a> · <a href="https://www.ebay.com/itm/376737690109" target="_blank">Side-view specimen</a></p>
      <h2>Working dimensions</h2><p>Shared 140.1 mm connector spacing, 27 mm bodies and 20 mm channel. The inlet is raised by one 47 mm stacking unit. Wheel: approximately 62 mm diameter, 18 mm wide, twelve pockets and six spokes. Axle is 66 mm along the 138 mm longitudinal span. Proportions follow USD290145 and specimen photos; physical measurements remain unverified.</p>
    </aside>
  </main>;
}
